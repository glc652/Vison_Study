...
class GrayscaleHistogramClassifier:
    ...
    def extract_histogram_features(self, image_path: str) -> np.ndarray:
        """提取灰度图像的直方图特征"""
        # 读取灰度图像
        ...
        for category in os.listdir(dataset_path):
            ...
            for filename in os.listdir(category_path):
                if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
                    ...
                    try:
                        # 提取特征
                        feature_vector = self.extract_histogram_features(image_path)
    ...
    def predict(self, image_path: str) -> Tuple[str, float]:
        ...
        # 提取特征
        features = self.extract_histogram_features(image_path)
...
def main():
    # 创建分类器实例
    classifier = GrayscaleHistogramClassifier(bins=64)

    # 加载模型
    model_path = r"D:\vision\mv\SVM\models\grayscale_histogram_svm_model.pkl"
    ...
    try:
        ...
        # 保存到Excel
        output_path = os.path.join(output_dir, "grayscale_histogram_test_results.xlsx")
