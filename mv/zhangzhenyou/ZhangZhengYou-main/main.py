...
# 超参数
data_dir         = r"D:\vision\mv\zhangzhenyou\used data\img"
output_dir       = r"D:\vision\mv\zhangzhenyou\output"
ends             = "bmp"
save_file_opencv = os.path.join(output_dir, "opencv.txt")
save_file_zhang  = os.path.join(output_dir, "zhang.txt")

points_per_row   = 11         # 每行棋盘格角点数量
points_per_col   = 8         # 每列棋盘格角点数量
square_size      = 15       # 单个棋盘格的物理尺寸大小(mm)
...
def main():
    ...
    try:
        ...
        error_zhang, A_zhang, k_zhang, _ = calibrator_zhang.calib(XYZ_all, uv_all)
        print(f"张正友标定成功")
        print(f"相机内参:\n{A_zhang}")
        print(f"畸变系数: {k_zhang}\n")
    except Exception as e:
        print(f"张正友标定失败: {e}")
        print("使用OpenCV结果代替")
        A_zhang = A_opencv
        k_opencv_flat = k_opencv.flatten() if k_opencv.ndim > 1 else k_opencv
        k_zhang = np.concatenate([k_opencv_flat[:2], np.zeros(max(0, 2 - len(k_opencv_flat[:2])))])
        error_zhang = error_opencv
